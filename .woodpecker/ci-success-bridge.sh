...
installation_id="$(api -H "Authorization: Bearer $jwt" "https://api.github.com/repos/${REPO}/installation" | jq -r '.id')"
token="$(api -X POST -H "Authorization: Bearer $jwt" "https://api.github.com/app/installations/${installation_id}/access_tokens" | jq -r '.token')"
description="Woodpecker pipeline ${STATE}"
api -X POST -H "Authorization: token $token" \
  "https://api.github.com/repos/${REPO}/statuses/${SHA}" \
  -d "$(jq -nc --arg state "$STATE" --arg context "$CONTEXT" --arg description "$description" --arg target_url "$TARGET_URL" '{state:$state, context:$context, description:$description} + (if $target_url=="" then {} else {target_url:$target_url} end)')" \
  >/dev/null
